...
let showBook = () => {
  ...
  arrBook.forEach((element) => {
    ...
    <h4>${element.title}</h4>
    <button data-id="${element.id}" class="countMinus">-</button>
    <p>${element.counter}</p>
    <button data-id="${element.id}" class="countPlus">+</button>
    </div>
    `;
  });
    ...
    .then((data) => {
      ...
      btnsBookAdd.forEach((element) => {
        element.addEventListener("click", () => {
          ...
          let sameId = arrBook.find((item) => item.id === find.id);
          if (sameId) {
